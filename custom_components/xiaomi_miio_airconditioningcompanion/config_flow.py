"""Config flow to configure Xiaomi Air Conditioning Companion component."""
import logging
...
async def validate_input(hass: core.HomeAssistant, data):
    ...
    """
    if data[CONF_TEMPERATURE_SENSOR]:
        states_source = hass.states.get(data[CONF_TEMPERATURE_SENSOR])
    if states_source is None:
        raise EntityNotExist

    if data[CONF_HUMIDITY_SENSOR]:
        states_source = hass.states.get(data[CONF_HUMIDITY_SENSOR])
    if states_source is None:
        raise EntityNotExist

    if data[CONF_POWER_SENSOR]:
        states_source = hass.states.get(data[CONF_POWER_SENSOR])
    if states_source is None:
        raise EntityNotExist
...
class OptionsFlowHandler(config_entries.OptionsFlow):
    ...
    async def async_step_init(self, user_input=None):
        ...
        if user_input is not None:
            ...
            cloud_username = self.config_entry.options.get(CONF_CLOUD_USERNAME)
            cloud_password = self.config_entry.options.get(CONF_CLOUD_PASSWORD)
            cloud_country = self.config_entry.options.get(CONF_CLOUD_COUNTRY)
            flow_type = self.config_entry.options.get(CONF_FLOW_TYPE)
            host = user_input.get(CONF_HOST)
            token = user_input.get(CONF_TOKEN)
            model = self.config_entry.options.get(CONF_MODEL)
            mac = self.config_entry.options.get(CONF_MAC)
        ...
        token = self.config_entry.options.get(CONF_TOKEN)
        model = self.config_entry.options.get(CONF_MODEL)
        ...
        if "acpartner" in model:
            temp_sensor = self.config_entry.options.get(CONF_TEMPERATURE_SENSOR, "")
            humidity_sensor = self.config_entry.options.get(CONF_HUMIDITY_SENSOR, "")
            power_sensor = self.config_entry.options.get(CONF_POWER_SENSOR, "")
            settings_schema = settings_schema.extend(
                {
                    vol.Required(CONF_HOST, default=host): str,
                    vol.Required(CONF_TOKEN, default=token): str,
                    vol.Optional(CONF_TEMPERATURE_SENSOR, default=temp_sensor): str,
                    vol.Optional(CONF_HUMIDITY_SENSOR, default=humidity_sensor): str,
                    vol.Optional(CONF_POWER_SENSOR, default=power_sensor): str,
                }
            )
...
class XiaomiACPartnerFlowHandler(config_entries.ConfigFlow, domain=DOMAIN):
    """Handle a Xiaomi Air Conditioning Companion config flow."""
